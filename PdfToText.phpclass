...
class  PdfToText 	extends PdfObjectBase
   {
	...
	protected function  ExtractTextWithLayout ( &$page_fragments, $page_number, $object_id, $data, &$current_font )
	   {
				...
				switch  ( ( $length  ===  1 ) ?  $token [0] : $token [1] ) 
				   {
					...
					// Td instruction
					case	'd' :
						$Tm [4]		+=  ( double ) $operand_stack [0]  ;
						$Tm [5]		+=  ( double ) $operand_stack [1] ;
		...
		usort ( $page_fragments, array ( $this, '__sort_page_fragments' ) ) ;
	    }
	...
	//	Sorts page fragments by their (y,x) coordinates, taking into account that a small variation in the y-position
	//	which is less than the current font height means that both texts should reside on the same line.
	public function  __sort_page_fragments ( $a, $b )
	   {
		$height			=  max ( $a [ 'font-height' ], $b [ 'font-height' ] ) ;
		$xa			=  $a [ 'x' ] ;
		$ya			=  $a [ 'y' ] ;
		$xb			=  $b [ 'x' ] ;
		$yb			=  $b [ 'y' ] ;

		if  ( $ya  !==  $yb  &&  abs ( $ya - $yb )  <=  $height )
			$yb	=  $ya ;
	...
	private function  __assemble_text_fragments ( $page_number, &$fragments, &$page_width, &$page_height )
	   {
		...
		if  ( isset ( $page_attributes [ 'width' ] )  &&  $page_attributes [ 'width' ] )
		   ...
		   {
			$page_width		=  0 ;
			$got_page_width		=  false ;
		    }

		if  ( isset ( $page_attributes [ 'height' ] )  &&  $page_attributes [ 'height' ] )
		   {
			$page_height		=  $page_attributes [ 'height' ] ;
			$got_page_height	=  true ;
		    }
		else
		   {
			$page_height		=  0 ;
			$got_page_height	=  false ;
		    }

		// Take the first text fragment
		$result			 =  '' ;
		$previous_fragment	 =  &$fragments [0] ;
		$font_object		 =  $this -> __compute_fragment_width ( $previous_fragment ) ;

		if  ( $this -> Options & self::PDFOPT_DEBUG_SHOW_COORDINATES )
			$result		.=  $this -> __debug_get_coordinates ( $fragments [0] ) ;

		$result			.=  $previous_fragment [ 'text' ] ;
		...
		for  ( $i = 1 ; $i  <  $fragment_count ; $i ++ )
		   {
			...
			$previous_width		=  $previous_fragment [ 'width' ] ;

			// Since we allow slight variations of the y-position (for example, because of super- or sub-scripted text), we retain the highest font 
			// height between the previous text fragment and the current one
			$font_height		=  max ( $previous_fragment [ 'font-height' ], $fragment [ 'font-height' ] ) ;

			// If the difference between both y-positions are greater than the current font height, then insert a newline
			if  ( abs ( $fragment [ 'y' ] - $previous_fragment [ 'y' ] )  >  $font_height )
				$result		.=  $this -> EOL ;
			// Otherwise, we may need to insert a space...
			else
			   {
				// If the width of the previous fragment could be computed, insert a space only if its x-position plus
				// the computed width is less than the x-position of the current text fragment
				if  ( $previous_width  !==  false )
				   {
					if  (  $previous_fragment [ 'x' ] + $previous_width  <  $fragment [ 'x' ] )
						$result		.=  $separator ;
					// However, in case of an ending x-position superior to the starting x-position of the current fragment,
					// we have to consider if they should really appear on the same line
					else
					   {
						$min_height		=  min ( $previous_fragment [ 'font-height' ], $fragment [ 'font-height' ] ) ;

						if  ( round ( $previous_fragment [ 'y' ] - $fragment [ 'y' ] )  >  ceil ( $min_height ) )
							$result		.=  $this -> EOL ;
					    }
				    }
				// No width information is associated with the current font
				else
				   {
					if  ( $font_object )
						$ending_x		 =  $previous_fragment [ 'x' ] + $font_object -> GetStringWidth ( $previous_fragment [ 'text' ],
																$this -> ExtraTextWidth ) ;
					else
						$ending_x		 =  0 ;

					if  ( $ending_x  <  $fragment [ 'x' ] )
						$result		.=  $separator ;
				    }
			    }
			...
			// Compute the width for the new previous fragment
			$font_object		=  $this -> __compute_fragment_width ( $previous_fragment ) ;
		...
		// Don't forget the compute the width of the very last fragment
		$fragment	=  &$fragments [ $fragment_count - 1 ] ;
		$this -> __compute_fragment_width ( $fragment ) ;

		// Update the page dimensions with the coordinates of the last text fragment, if they were not specified in the PDF file
		if  ( ! $got_page_width )
		   {
			$w	=  $previous_fragment [ 'x' ] + $previous_fragment [ 'width' ] ;
		...
		if  ( ! $got_page_height )
		   {
			$h	=  $previous_fragment [ 'y' ] + $previous_fragment [ 'font-height' ] ;
		...
		if  ( $this -> Options  &  self::PDFOPT_DEBUG_SHOW_COORDINATES )
			$result		=  "[Page : $page_number, width = $page_width, height = $page_height]" . $this -> EOL . $result ;
	...
	public function  Add ( $object_id, $font_definition, $pdf_objects, $extra_mappings )
	   {
		...
		$this -> Fonts [ $object_id ]	=  new  PdfTexterFont ( $object_id, $cmap_id, $font_type, $secondary_cmap_id, $pdf_objects, $extra_mappings ) ;
...
class  PdfTexterFont		extends PdfObjectBase
   {
	...
	public function  __construct ( $resource_id, $cmap_id, $font_type, $secondary_cmap_id = null, $pdf_objects = null, $extra_mappings = null )
	   {
		parent::__construct ( ) ;
		...
		switch  ( $this -> FontType )
		   {
			...
			case	self::FONT_ENCODING_PDF_MAP :
				$this -> CharacterMap	=  new  PdfTexterEncodingMap ( $cmap_id, $this -> FontVariant, $extra_mappings ) ;
				break ;
		...
		// So ensure they are all there (not that /MissingWidths can be absent)
		if  ( $first_char  &&  $widths )
		   {
			...
			for  ( $i = 0, $count = count ( $widths ) ; $i  <  $count ; $i ++ )
			   {
				...
				$this -> CharacterWidths [ chr ( $first_char + $i ) ]	=  ( $value ) ?  $value : $this -> DefaultWidth ;
			    }
